...
class Edge:
    def __init__(self, vertex1, vertex2, capacity, isResidual, returnEdge=None):
...
class Graph:
    ...
    def setSink(self, sink: Vertex):
        self.sink = sink

    def setSource(self, source: Vertex):
        self.source = source
...
class AdjListGraph(Graph):
    ...
    def insertEdge(self, vertex1, vertex2, capacity):
        ...
        forwardEdge = Edge(vertex1, vertex2, capacity, False, None)
        backwardEdge = Edge(vertex2, vertex1, capacity, True, forwardEdge)
        forwardEdge.backwardEdge = backwardEdge

        self.adjList[index1][vertex2] = forwardEdge

        if(not self.adjList[index2].get(vertex1, None)):
            self.adjList[index2][vertex1] = backwardEdge
